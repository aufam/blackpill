...
namespace Project::DSP {
    ...
    struct amplitude8_t { // float val is -1.0 to 1.0
        ...
        explicit operator float() const { return lut_f8[this->u]; }
    };

    struct angle32_t { // float val is -pi to pi
        int32_t val;
        ...
        constexpr angle32_t operator +(const angle32_t &other) const { return angle32_t{ this->val + other.val }; }
        constexpr angle32_t operator -(const angle32_t &other) const { return angle32_t{ this->val - other.val }; }

        constexpr angle32_t operator +(const int32_t &other) const { return angle32_t{ this->val + other }; }
        constexpr angle32_t operator -(const int32_t &other) const { return angle32_t{ this->val - other }; }

        constexpr angle32_t operator +(const float &other) const { return angle32_t{this->val + floatToFix(other) }; }
        constexpr angle32_t operator -(const float &other) const { return angle32_t{this->val - floatToFix(other) }; }
        ...
        angle32_t &operator +=(const float &other) {
            this->val = this->val + floatToFix(other);
            return *this;
        }

        amplitude8_t sin() const {
            auto index = (this->val >> 24) & 0xFF;
            return amplitude8_t{ lut_sin[index] };
        }
        amplitude8_t cos() const {
            auto index = ((this->val >> 24) + 64) & 0xFF;
            return amplitude8_t{ lut_sin[index] };
        }
    };
    ...
    struct complex {
        ...
        float getReal() const { return float(amplitude8_t(real)); }
        float getImag() const { return float(amplitude8_t(imag)); }

        float atanf() const     { return atan2f32(imag, real); }
        angle32_t atani() const { return { this->atanf() }; }

        complex<int32_t> multiplyConjugate(const complex<T> &other) const {
            return {
